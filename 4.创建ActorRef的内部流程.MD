...
> 接下来看看初始化LocalActorRef实例时，做了哪些事?＜/br＞
1. 创建底层的ActorCell实例＜/br＞
2. 调用ActorCell的init方法＜/br＞
   ① 为ActorCell创建Mailbox实例 (通过MailboxType的create方法创建MessageQueue实现类实例＜/br＞
   ② 将当前的ActorCell实例赋给Mailbox的actor字段＜/br＞
即：＜/br＞
**ActorCell持有它的Mailbox实例**。这样在向actor发送消息时，可以通过actor底层的ActorCell获得Mailbox。＜/br＞
**Mailbox也持有ActorCell实例**。这样在将Mailbox交给线程池执行时，从Mailbox取出的消息就可以交给对应的ActorCell处理。＜/br＞
...
1. 将Actor实例与`消息派发器绑`定在一起＜/br＞
2. 将Actor的`Mailbox`交给线程池中的线程执行 (之后的流程在`5.Akka消息发送流程`一节中有介绍＜/br＞
